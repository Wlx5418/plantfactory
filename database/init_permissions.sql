-- 初始化权限数据
INSERT INTO permissions (name, code, description, resource_type, resource_path, http_method, parent_id, sort_order, created_at, updated_at) VALUES
-- 用户管理权限
...
-- 初始化角色数据
INSERT INTO roles (role_name, role_code, description, status, created_at, updated_at) VALUES
('系统管理员', 'ADMIN', '拥有系统所有权限的超级管理员', 'ACTIVE', NOW(), NOW()),
('操作员', 'OPERATOR', '负责日常操作和设备控制', 'ACTIVE', NOW(), NOW()),
('观察员', 'OBSERVER', '只能查看数据，无操作权限', 'ACTIVE', NOW(), NOW()),
('管理员', 'MANAGER', '部门管理员，具有部分管理权限', 'ACTIVE', NOW(), NOW());
...
WHERE r.role_code = 'OPERATOR' AND p.code IN (
    'environment_monitor', 'environment_view', 'environment_history',
    'device_control', 'device_operate', 'device_status',
    'data_view', 'area_manage', 'area_create', 'area_edit', 'area_delete'
);

-- 观察员权限
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permissions p
WHERE r.role_code = 'OBSERVER' AND p.code IN (
    'environment_monitor', 'environment_view', 'environment_history',
    'data_view', 'log_view'
);
...
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permissions p
WHERE r.role_code = 'MANAGER' AND p.code IN (
    'user_manage', 'user_view', 'user_create', 'user_edit', 'user_status',
    'role_manage', 'role_view', 'role_create', 'role_edit', 'role_permission',
    'environment_monitor', 'environment_view', 'environment_history',
    'device_control', 'device_operate', 'device_status',
    'data_view', 'data_export', 'area_manage', 'area_create', 'area_edit', 'area_delete',
    'log_view'
);